...
###### (**) Your code must export the contents in a function which passes ``bot, msg, timeout, botPerm, userPerm`` under ``exports.main``, aswell as a command description under ``exports.desc`` and a command syntax under ``exports.syntax``, see existing commands for examples.*
