...
fs.readdirSync(normalizedPath).forEach(function(file) { // Look at all the files in the specificed folder
	...
	// If the file is a .json file...
		var ServerID = file.slice(0, -5).toLowerCase();  // ...remove ".json" bit from the file names, convert it to lowercase...
		ignoreLists[ServerID] = require("./"+config.ignorePath+"/" + file); // ...and then require the files as ignore lists.
	};
});
