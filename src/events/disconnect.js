...
function disconnect(client, event) {
	if (event.code === 1000) {
		client.logger.info(`Automatically restarting due to WS error code ${event.code} (${event.reason})...`);
		...
		client.logger.error(`Disconnected with WS error code ${event.code} (${event.reason}), exiting!`);
		// Stop the pm2 task on other critical errors, exit process if no pm2
		return client.config.pm2 ? exec('pm2 stop robbot') : process.exit(0);
	}
}
