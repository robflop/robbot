...
bot.on('message', msg => { // listen to all messages sent
	...
	if(msg.content.startsWith(config.commandPrefix)) { // Only listen to messages starting with the bot prefix
		...
		if(ignoreList.indexOf(msg.author.id) == -1) { // Check if the user is on the ignore list ("-1" meaning they are not, anything above means they are)
			...
			if(msg.content.indexOf("setAvatar") - config.commandPrefix.length == 1) { // Check if "setAvatar" comes right after the bot prefix, with one space inbetween
				if(permission.hasPermission('SEND_MESSAGES')) { // Check if bot can send messages to the channel 
					...
					if(msg.author.id !== config.ownerID) { // Check for authorization
						...
						bot.user.setAvatar(msg.content.substr(config.commandPrefix.length + command.length + 2));
						/* 
						...
						*/
						msg.reply(`successfully set my avatar to '${msg.content.substr(config.commandPrefix.length + command.length + 2)}' ! \n(May not have worked if ratelimit capped)`);
			...
			if(msg.content.indexOf("setName") - config.commandPrefix.length == 1) { // Check if "setName" comes right after the bot prefix, with one space inbetween
				if(permission.hasPermission('SEND_MESSAGES')) { // Check if bot can send messages to the channel 
					...
					if(msg.author.id !== config.ownerID) { // Check for authorization
						...
						bot.user.setUsername(msg.content.substr(config.commandPrefix.length + command.length + 2));
						/* 
							...
							function (error, response, body) {
								if(error || response.statusCode !== 200) { // Check for errors or response codes other than 200 (OK)
									...
									if(permission.hasPermission('SEND_MESSAGES')) { // Check if bot can send messages to the channel 
										...
										// If it can't PM, the author of the msg
									}
									return; // abort command execution
								};
			...
			if(msg.content.indexOf('showLog') - config.commandPrefix.length == 1) {
				...
				if(msg.author.id !== config.ownerID) { // Check for authorization
					...
					var file = msg.content.substr(config.commandPrefix.length + command.length + 2); // define the argument of the command as file variable
					if(msg.content.length == config.commandPrefix.length + command.length + 2) { // If the command is given without argument (file) ...
						msg.author.sendMessage("Specify a log to display!"); // tell the author to specify a file ..
						return; // .. and abort command execution.
					}
					else {
						if(file.startsWith(".") || file.startsWith("/") || file.startsWith("\\") || file.startsWith("&") || file.indexOf("%") > -1 || file.indexOf("config.json") > -1 || file.indexOf("/../") > -1) { // Security measures to prevent reading files other than logs
							msg.author.sendMessage("I'm sorry, but I can't let you do that. (Banned character or string in argument)"); // reject the command call
							fs.appendFileSync(`${config.logPath}${config.serverLog}`, `\n[${moment().format('DD/MM/YYYY HH:mm:ss')}][SHOWLOG] ${msg.author.username}#${msg.author.discriminator} tried using the "${msg.content.substr(config.commandPrefix.length + 1, command.length)}" command  on the '${msg.guild}' server, but provided a banned character/string!`); 
							// Log command use, when and by whom
							return; // abort command execution
						}
						fs.readFile(`${config.logPath + file}`, "utf-8", (error, data) => { // read the given argument file
							if(error) {
		...
		else { return; } // Ignore author if on the ignore list (duh)
	}
